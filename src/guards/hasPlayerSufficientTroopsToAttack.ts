...
export const hasPlayerSufficientTroopsToAttack = ({ context, event }: RiskGuard<SelectTroopsEvent>) => {
	const isValid =
		(event.troops > 0 &&
			...
			context.ownership[context.fromTerritory].troops > event.troops &&
			context.ownership[context.fromTerritory].player !== context.ownership[context.toTerritory as Territory].player) ??
		false;
